"""
模型训练脚本 - 数据集2 (Dataset2)

训练TF-IDF + 逻辑回归分类模型
Train TF-IDF + Logistic Regression classification models for Dataset2
"""
...
class ModelTrainer:
    ...
    def train_field_model(self, field_name: str, max_features: int = 500, c: float = 1.0):
        ...
        # 保存分类器
        classifier.vectorizer = feature_engine
    ...
    def train_all_models(self):
        ...
        """
        fields = [
            {"name": "domain", "max_features": 2000, "c": 1.0},
            {"name": "experiment_type", "max_features": 5000, "c": 1.0},
            {"name": "dataset_type", "max_features": 2000, "c": 1.0},
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="训练TF-IDF + 逻辑回归分类模型 (数据集2)")
    ...
    model_dir = ML_CLASSIFICATION / "models" / "dataset2"
