"""
datasets--策略v4.2--Union Ensemble

通道 A: v4.1（不变）
通道 B: v4 宽松候选 → v3 Gazetteer 硬过滤
合并: gazetteer 等价类去重 union，冲突时保留 v4.1 条目
"""
...
def _dedupe_key(name: str, alias_groups: dict[str, set[str]]) -> str:
    """Map name to gazetteer equivalence class key, or normalize_fuzzy fallback."""
    for label in (name,):
...
def _merge_union(
    ...
    """
    Union v4.1 datasets with gazetteer-filtered canonical names.
    Prefer v4.1 metadata when keys collide.
    """
...
class DatasetRuleV42:
    """Dataset extraction v4.2: v4.1 union (v4 loose → gazetteer hard filter)."""
    ...
    def extract(paper_md: str, paper_id: str = "") -> dict[str, Any]:
        ...
        loose_names, loose_trace = DatasetRuleV4.extract_loose_candidate_names(paper_md)
...
"""
    out = DatasetRuleV42.extract(sample, "test")
