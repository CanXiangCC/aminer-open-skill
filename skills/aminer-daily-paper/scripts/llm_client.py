...
SYSTEM_PROMPT = """假如你是推荐机器人，请基于后端已经返回的论文信息，对用户可见的摘要内容做润色与统一表达，确保输出包含以下字段：keywords, structured_summary, famous_authors。
...
- 优先参考输入里的 backend_structured_summary 和 backend_summary，对已有内容做润色、压缩和去重，不要随意改写原意
- abstract 只作为校对和补充依据，不要忽略后端已经给出的 summary / structured_summary
- 如果论文不包含明确实验结果，可省略 experimental_results
- 如果 backend_summary、backend_structured_summary、abstract 都不足以支持可靠总结，禁止根据 title 臆测论文内容；这时应明确说明信息不足
...
backend summary:
{backend_summary}
backend structured summary:
{backend_structured_summary}
famous authors:
{known_authors}
comment: {comment}
"""
...
def _require_string_list(value: Any, field_name: str, raw_output: str) -> list[str]:
    # Accept comma-separated string (common LLM output format)
    if isinstance(value, str):
        value = [piece.strip() for piece in re.split(r"[,，;；、]+", value) if piece.strip()]
...
def build_summary_prompt(paper: dict[str, Any]) -> str:
    ...
    backend_structured_summary = paper.get("structured_summary")
    if isinstance(backend_structured_summary, dict):
        structured_summary_text = format_structured_summary(
            {
                key: _clean_summary_text(value)
                for key, value in backend_structured_summary.items()
    ...
    return USER_PROMPT_TEMPLATE.format(
        ...
        backend_summary=str(paper.get("summary") or "").strip(),
        backend_structured_summary=structured_summary_text,
