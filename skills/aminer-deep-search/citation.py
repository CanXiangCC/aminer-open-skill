...
def _auth_headers() -> dict[str, str]:
    return {"Authorization": f"Bearer {_get_aminer_key()}"}


def _json_auth_headers() -> dict[str, str]:
    return {
        "Content-Type": "application/json;charset=utf-8",
        "Authorization": f"Bearer {_get_aminer_key()}",
    }
...
def _normalize_paper_detail(detail: dict[str, Any], *, query: str = "") -> dict[str, Any]:
    ...
    normalized["score"] = round(_rule_based_score(normalized, query=query), 4)
    return normalized
...
def _rule_based_score(paper: dict[str, Any], *, query: str = "") -> float:
...
def _aminer_get_paper_info_batch(
...
def get_reference_papers(
    ...
    *,
    ...
    details = _aminer_get_paper_info_batch(ordered_ids)
    detail_by_id = {
    ...
    for paper_id in ordered_ids:
        ...
        if not detail:
            continue
        normalized = _normalize_paper_detail(detail, query=topic)
        normalized["score"] = round(_rule_based_score(normalized, query=topic), 4)
